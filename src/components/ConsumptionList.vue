<template>
    <div>
        <!-- 搜索框 -->
        <div style="display: flex;position:relative;min-width:1400px;margin-top:70px">
            <el-form size="mini" :inline="true" class="demo-form-inline" style="display: flex; flex-wrap: wrap;" :disabled="loading">
                ...
                <el-form-item label="推广" class="noselect" style="margin-right: 30px;" prop="new_user">
                    <el-select clearable style="width:150px" v-model="searchParams.user" placeholder="筛选" filterable>
                ...
                <el-form-item label="状态" class="noselect" style="margin-right:30px">
                    <el-select v-model="searchParams.project" placeholder="筛选" style="width:150px">
                        <el-option v-for="item in [{value: 2,label: '全部'},{value: 1,label: '已申报'}, {value: 0,label: '未申报'}]" :key="item.value" :label="item.label" :value="item.value"></el-option>
                    </el-select>
                </el-form-item>
                ...
                <el-form-item class="noselect">
                    <el-button class="info_btn iconfont icon-bianji" @click="exchangerate_form_dialogVisible = true" type="info" style="font-size:12px;height: 30px; position: relative;right:-10px;margin-left:10px;" > 汇率设置</el-button>
                    <el-button :disabled="this.standBy_search === null" class="info_btn" icon="el-icon-download" @click="exportExcel" type="info" style="height: 30px; position: relative;right:-10px;margin-left:20px;" >导出</el-button>
    ...
    <el-table
        ...
        :data="ShowTableData"
        tooltip-effect="dark"
        style="width:calc(100%-60px);line-height:30px;margin:5px 0px;min-width:1370px;margin-bottom:25px"
        :row-style="{height: '50px'}"
        border
        @selection-change="handleSelectionChange_delete">
        <el-table-column prop="expend_date" label="消耗日期" width="100px">
        </el-table-column>
        <el-table-column prop="apply_date" label="申报时间" width="100px">
            <template slot-scope="scope">
                <div v-if="scope.row['apply_date'] != null && scope.row['apply_date'] != ''">{{scope.row["apply_date"]}}</div>
                <div v-else>/</div>
            </template>
        </el-table-column>
        <el-table-column label="申报状态" width="80px">
            <div>审核中</div>
        </el-table-column>
        <el-table-column prop="zh_name" label="申报人员" width="180px">
            <template slot-scope="scope">
                <div style="display:inline">{{scope.row["zh_name"]}} </div>
                <span> ({{scope.row["user"]}})</span>
            </template>
        </el-table-column>
        <el-table-column prop="account" label="账户名称" width="350px">
            <template slot-scope="scope">
                <div>{{scope.row["account_name"]}}</div>
                <div v-if="scope.row['total'] != null && scope.row['total'] != '' && scope.row['total'] != undefined"><span style="color:#f56c6c;margin-right:20px;font-size: 12px;font-weight: bolder;">已停用</span><span style="color:#f56c6c;margin-right:10px;font-size: 12px;font-weight: bolder;">已报合计：USD </span> {{scope.row["total"]}}</div>
            </template>
        </el-table-column>
        <el-table-column prop="ad_id" label="广告账号ID" width="200px">
        </el-table-column>
        <el-table-column prop="project" label="项目/地区" width="160px">
            <template slot-scope="scope">
                <div v-if="scope.row['project'] != null && scope.row['project'] != ''">{{scope.row["project"]}}</div>
                <div v-else>/</div>
            </template>
        </el-table-column>
        <el-table-column  label="消耗金额" width="120px">
            <template slot-scope="scope">
                <div>
                    <span style="color:#f56c6c;margin-right:10px;font-size: 12px;font-weight: bolder;">{{scope.row["currency"]}}</span>
                    <span v-if="scope.row['expend_money'] != null">{{scope.row["expend_money"]}}</span>
                    <span v-else>0.00</span>
                </div>
                <div>
                    <span style="color:#f56c6c;margin-right:10px;font-size: 12px;font-weight: bolder;">RMB</span>
                    <span style="position:relative;left:-2px">{{scope.row["expend_money_RMB"]}}</span>
                </div>
            </template>
        </el-table-column>
        <el-table-column prop="exchange_rate" label="汇率" width="100px">
            <template slot-scope="scope">
                <div v-if="scope.row['exchange_rate'] != null && scope.row['exchange_rate'] != ''">{{scope.row["exchange_rate"]}}</div>
                <div v-else>/</div>
            </template>
        </el-table-column>
    ...
    <!--汇率表单  -->
    <el-dialog :close-on-click-modal="false" :visible.sync="exchangerate_form_dialogVisible" width="600px" label-width="80px">
        <!-- <el-form label-width="100px" label-position="right"> -->
            <!-- <el-form-item> -->
        <el-button :disabled="this.exchangerate_loading" style="position:relative;left:-220px" size="small" type="primary" @click="new_exchangerate_dialogVisible = true" >新 增</el-button>
            <!-- </el-form-item>
        </el-form> -->
        <el-table v-loading="exchangerate_loading" element-loading-spinner="el-icon-loading" element-loading-background="rgba(255,255,255,0.5)" :data="ShowTableData_exchangerate" tooltip-effect="dark" style="width: calc(100% - 60px); line-height: 30px; margin:30px;margin-top: 25px" border>
            <el-table-column prop="currency" label="货币缩写">
            </el-table-column>
            <el-table-column prop="currency_zh" label="货币全称">
            </el-table-column>
            <el-table-column prop="num" label="汇率">
            </el-table-column>
            ...
            <template slot-scope="scope">
                ...
                <el-dropdown-menu :visible-arrow="false" class="noselect" slot="dropdown">
                    <el-dropdown-item type="text" size="small" @click.native="edit_exchangerate_dialog(scope.row)">编辑项目</el-dropdown-item>
                    <el-dropdown-item id="delAd" type="text" size="small" @click.native="del_exchangerate(scope.row)">删除项目</el-dropdown-item>
                </el-dropdown-menu>
                </el-dropdown>
            </template>
            </el-table-column>
...
import { export_json_to_excel } from '../Export2Excel.js'
    export default {
        ...
        methods: {
            ...
            // 汇率列表获取
            async fetch_exchangerate() {
                this.exchangerate_loading = true
                await this.$http.get('/index.php/index/account/rateList/').then((res) => {
                    res.data["data"].forEach(e => {
                        e["update_time"] = utils.getLocalTime(parseInt(e["update_time"]));
                    })
                    if (this.ShowTableData.length != 0) {
                        this.ShowTableData.forEach(e=> {
                            ...
                            res.data['data'].forEach(i=> {
                                if (i["currency"] === e["currency"]) {
                                    e["exchange_rate"] = i["num"]
                                }
                            })
                            e["expend_money_RMB"] = e["expend_money"] * e["exchange_rate"]
                            e["expend_money_RMB"] = e["expend_money_RMB"].toFixed(2)
                        })
                    }
                    this.ShowTableData_exchangerate = res.data['data']
                    this.exchangerate_loading = false
                })
            },
            // 汇率设置
            ...
            // 删除汇率
            async del_exchangerate(e) {
                this.$confirm(`是否删除该项目: ${e.currency_zh}`, "提示", {
                    ...
                    .then(async () => {
                        ...
                        await this.$http.post("/index.php/index/account/deleteRete/",formData).then((res) => {
                        if (res.data["code"] === 200) {
                            this.fetch_exchangerate()
                            this.$message.success(`${res.data["data"]}`)
                        } else {
            ...
            exportExcel() {
                ...
                .then(async () => {
                    ...
                    var tHeader = ['推广','账号','账号ID','信用卡号','消耗日期','消耗金额','币种','状态','汇率','人民币','更新时间','项目','备注']
                    var filterVal = ['zh_name','account','ad_id','card_id','expend_date','expend_money','currency','audit_type','exchange_rate','expend_money_RMB','apply_date','project','remark']
                    var time = this.getCurentTime()
                    var filename = `${time}`
                    arr.forEach(e=> {
                        e["expend_date"] = utils.getLocalTime(parseInt(e["expend_date"]));
                        e["apply_date"] = utils.getLocalTime(parseInt(e["apply_date"]));
                        this.ShowTableData_exchangerate.forEach(i=> {
                            if (i["currency"] === e["currency"]) {
                                e["exchange_rate"] = i["num"]
                            }
                        })
            ...
            async handleSizeChange(val) {// 页大小
                if (this.searchPageChange === false && this.useSearchData === false) {
                    ...
                    this.currentPage = 1
                    this.pageSize = val
                    this.tableHeight = utils.setClientHight(this,230,56,78)
            ...
            //搜索提交
            async search(val) {
                this.searchParams["apply"] = null
                this.searchParams["audit"] = null
                this.loading = true
                ...
                if (this.searchParams["project"] === 0) {
                    this.searchParams["apply"] = 1
                } else if (this.searchParams["project"] === 1) {
                    this.searchParams["audit"] = 1
                } else if (this.searchParams["project"] === 2) {
                    this.searchParams["audit"] = 1
                    this.searchParams["apply"] = 1
                }
                let formData = new FormData()
                ...
                // 搜索后换页/页大小
                    ...
                    this.standBy_search["apply"] = null
                this.standBy_search["audit"] = null
                this.standBy_search["page"] = val
                this.standBy_search["num"] = this.pageSize
                if (this.standBy_search["project"] === 0) {
                    this.standBy_search["apply"] = 1
                    delete this.standBy_search.audit

                } else if (this.standBy_search["project"] === 1) {
                    this.standBy_search["audit"] = 1
                    delete this.standBy_search.apply
                } else if (this.standBy_search["project"] === 2) {
                    this.standBy_search["audit"] = 1
                    this.standBy_search["apply"] = 1
                }
                let formData = new FormData()
        ...
        watch: {
            clientHeight_watch (val) {
                var _this = this;
                _this.tableHeight = utils.setClientHight(_this,230,56,78)
            }
        },
    }
</script>
