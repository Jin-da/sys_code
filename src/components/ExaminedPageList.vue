<template>
  <div>
    <!-- 搜索框 -->
    <div v-if="this.authority['createexaminedpage']" style="display: flex;position:relative;min-width:915px">
      <el-form size="mini" :inline="true" class="demo-form-inline" style="display: flex; flex-wrap: wrap" :disabled="loading">
        <el-form-item class="noselect">
          <el-button icon="el-icon-plus" class="info_btn" @click="newPage()" type="info" style="height: 30px; position: relative; margin-top: 32px">创建屏蔽页</el-button>
        </el-form-item>
      </el-form>
    </div>
    <!-- 卡片 -->
      ...
      <el-card class="list" v-for="(item,index) in this.ShowTableData" :key="index" :body-style="{ padding: '0px' }">
        ...
        <div style="padding: 14px;">
          ...
          <div class="bottom clearfix" style="position:relative">
            ...
            <el-button v-if ="authority.editexaminedpage" type="text" class="button" @click="edit(item)">编辑</el-button>
          </div>
        </div>
      </el-card>
    ...
    <!-- 编辑屏蔽页表单 -->
    <el-dialog :close-on-click-modal="false" title="编辑屏蔽页" :visible.sync="edit_form_dialogVisible" width="450px">
      <el-form label-width="100px" label-position="right" v-loading="edit_form_loading">
        <el-row>
          <el-col :span="24">
            <el-form-item label="屏蔽页名称" class="noselect" style="margin-right: 40px" prop="ad_title">
    ...
    <!-- 创建屏蔽页表单 -->
    <el-dialog :close-on-click-modal="false" title="创建屏蔽页" :visible.sync="new_form_dialogVisible" width="450px">
...
export default {
  ...
  methods: {
    ...
    clickImg(e) {//图片跳转预览
      window.open(`http://move.icethem01.xyz/safePage/${e}`)
    },
    getUid() { // 获取用户uid
