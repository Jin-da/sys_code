  ...
  <div>
    ...
    <div style="display: flex;position:relative;min-width:855px">
      <el-form size="mini" :inline="true" class="demo-form-inline" style="display: flex; flex-wrap: wrap;width:100%;justify-content: space-between" :disabled="loading">
        ...
        <el-form-item class="noselect">
          <el-button icon="el-icon-delete" @click="resume" class="getDomain" type="info" style="height: 30px; position: relative; margin-top: 32px;margin-left:25px;">域名恢复</el-button>
    ...
    <el-table
      ...
      <el-table-column prop="create_time_change" label="申请时间" width="220px"></el-table-column>
      <el-table-column prop="domain_name" label="域名" width="210"></el-table-column>
      <el-table-column prop="expiring_time_change" label="预估过期时间" width="220px"></el-table-column>
      <el-table-column prop="price" label="价格" width="130px"></el-table-column>
      <!-- <el-table-column prop="show_bt_id" label="用于" width="180px"></el-table-column> -->
      <el-table-column prop="create_time_change" label="状态" width="130px">
        <template slot-scope="scope">
          <div v-if="scope.row['sell'] === '0'">正常</div>
          <div v-else>已出售</div>
        </template>
      </el-table-column>
      <el-table-column prop="expiring_time_change" label="网域验证" width="130px">
        <template slot-scope="scope">
      ...
      <el-table-column label="强制HTTPS" width="130px">
        <template slot-scope="scope">
      ...
      <el-table-column prop="remark" label="备注" min-width="250px"></el-table-column>
      <el-table-column fixed="right" label="操作" align="center" width="130px">
        <template slot-scope="scope">
          <!-- <p id="lock" v-if="scope.row.https_site != 'off'" class="el-icon-circle-check" style="margin-right:10px;color:#67C23A;display:inline-block;font-size:15px;top:15px" type="text"></p> -->
          <el-dropdown placement="bottom" trigger="click">
            ...
            <el-dropdown-menu :visible-arrow="false" class="noselect" slot="dropdown">
              ...
              <el-dropdown-item @click.native="newSubDomain(scope.row)" :disabled="scope.row['sell'] === '0'?false:true">添加二级域名</el-dropdown-item>
              <el-dropdown-item v-if="scope.row.fb_check === 0" @click.native="domainAuthentication(scope.row)" :disabled="scope.row['sell'] === '0'?false:true">网域验证</el-dropdown-item>
              <el-dropdown-item v-if="scope.row.fb_check != 0" disabled>网域已验证</el-dropdown-item>
              ...
              <el-dropdown-item id="del" @click.native="remove(scope.row)">删除域名</el-dropdown-item>
            </el-dropdown-menu>
          </el-dropdown>
        </template>
      </el-table-column>
    ...
    <!-- 详情抽屉 -->
    <el-drawer title="详情" :visible.sync="table" direction="rtl" size="650px">
      <el-form ref="makeOver_form" label-width="100px" label-position="left" style="padding-left:70px;min-height:500px" v-loading="loadDetail">
        <el-form-item size="small" label="子域名: ">
          <div style="display:flex;flex-wrap: wrap;justify-content:start;width: 400px;" v-for="(item,index) in detail_subDomain" :key="index" :label="item['domain']" :value="item['domain']">
            <!-- <span style="text-align:left;">
              <span style="text-decoration:none;">域名：<span>{{item['domain']}}</span> / </span>服务器：{{item['show_bt_id']}}</span> -->
              <p style="width: 180px;text-align: left;">{{item['domain']}}</p>
              <p>服务器：{{item['show_bt_id']}}</p>
          </div>
          <p style="text-align:left;" v-if="detail_subDomain.length === 0">无</p>
        </el-form-item>
        <el-form-item size="small" label="已删除子域名: ">
          <template slot-scope="scope">
            <div style="display:flex;flex-wrap: wrap;justify-content:start;width: 400px;" v-for="(item,index) in detail_subDomain_del" :key="index" :label="item['domain']" :value="item['domain']">
              <!-- <p style="text-align:left;"><el-button style="margin-right:10px;font-size:14px;" type="text" @click="renew(scope.row,item)">恢复</el-button><span style="text-decoration:none;"><span>{{item['domain']}}</span> / </span>服务器：{{item['show_bt_id']}}</p> -->
              <p style="width: 200px;text-align: left;"><el-button style="margin-right:10px;font-size:14px;" type="text" @click="renew(scope.row,item)">恢复</el-button>{{item['domain']}}</p>
              <p>服务器：{{item['show_bt_id']}}</p>
            </div>
            <p style="text-align:left;" v-if="detail_subDomain_del.length === 0">无</p>
          </template>
        </el-form-item>
      </el-form>
    </el-drawer>
    ...
    <el-dialog title="新增子域名" :visible.sync="newSubmitDomain_dialogVisible" width="450px" :close-on-click-modal="false" @close='closeDialog'>
      <el-form :rules="rules" ref="newSubmitDomain_value" :model="newSubmitDomain_value" label-width="100px" label-position="right" v-loading="newSubDomain_loading">
        <el-row>
          <el-col :span="24">
            <el-form-item label="服务器" class="noselect" style="margin-right: 40px" prop="server">
...
export default {
  ...
  methods: {
    async renew(e,item) {
      ...
      formData.append('domain',item["zone_name"])
      await this.$http.post('/index.php/index/domain/recoverDomain',formData).then((res) => {
    ...
    async mainDomainServer_submit() {
      ...
      await this.$http.post('/index.php/index/cfapi/createRecord/',formData).then((res) => {
        if (res.data["code"] === 200) {
          ...
          // console.log(this.standBY_e_mainDomain["show_bt_id"]);
      ...
      submitSearchParams.domain = this.searchParams.accurateSearch;
        ...
        await this.$http.post("/index.php/index/domain/domainSearch/",formData)
          .then((res) => {
            ...
            e["expiring_time_change"] = utils.getLocalTime(
              parseInt(e["expiring_time"])
            ); // 时间戳转换
          })
          this.ShowTableData =[]
    ...
    async tableOpen(e) {// 详情
      ...
      // .then((res) => {
        e["subDomain"].forEach(i=> {
          ...
          if(i["delete"] === 1) {
            this.detail_subDomain_del.unshift({"domain":`${`${i["top"]}`+ '.' + i["zone_name"]}`,"show_bt_id":`${i['show_bt_id']}`,"top":`${i["top"]}`,"zone_name":`${i["zone_name"]}`})
          } else {
            this.detail_subDomain.unshift({"domain":`${`${i["top"]}`+ '.' +i["zone_name"]}`,"show_bt_id":`${i['show_bt_id']}`})
          }
        })
        // this.loadDetail = false
      // });
    ...
    async getDomainDialogSubmit() {// 申请域名表单提交
      ...
      await this.$http.post('/index.php/index/domain/applydomain/',formData).then((res) => {
        if (res.data["code"] === 200) {
